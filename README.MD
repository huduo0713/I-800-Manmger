...
- 🌐 **智能设备注册**: 自动网卡检测、动态设备ID生成、Runtime状态监测
...
### � v2.4.0 - OPC UA集成与协议统一化 (2025-09-26)
- 🆕 **OPC UA端口配置**: 新增device.opcua.serverPort配置，支持OPC UA服务器端口管理
- 🆕 **设备注册增强**: 注册消息自动包含opcuaServerPort字段，便于平台统一管理
- 🆕 **启动日志优化**: 显示OPC UA服务地址信息 (IP:端口)
- 🔧 **协议标准化**: 设备注册字段名统一从params改为data，符合平台规范
- 🔧 **结构体重构**: DeviceRegisterRequestParams → DeviceRegisterRequestData
- 📝 **文档同步**: 更新设备注册JSON示例和OPC UA配置说明
...
- 🆕 **Runtime状态检测**: 实时监测Runtime进程状态，上报runtimeStatus字段
- 🆕 **ClientID优化**: edge-{MAC地址} 格式，符合设备命名规范
- 🛠️ **连接状态修复**: 正确区分首次连接和重连，修复连接类型显示问题
- ⚙️ **配置增强**: 新增device.network、device.runtime和mqtt.register配置节点
...
### 功能说明
设备注册时会自动检测Runtime进程状态，通过监测指定端口的监听状态来判断Runtime进程是否正常运行。
...
device:
  runtime:
    port: 1231              # Runtime进程监听端口，默认1231
```

### 检测机制
- **检测方式**: 使用`net.DialTimeout`尝试连接本地端口
- **超时时间**: 100ms，平衡检测速度和准确性
- **返回值**:
  - `1`: Runtime进程正常运行（端口在监听）
  - `0`: Runtime进程停止或异常（端口未监听）

### 上报字段
设备注册时会在MQTT消息中包含`runtimeStatus`和`opcuaServerPort`字段：
```json
{
  "data": {
    "deviceModule": "I-800-RK",
    "deviceId": "F2-D5-4F-C7-2B-AD",
    "heartBeat": 10,
    "IP": "192.168.11.204",
    "runtimeStatus": 1,
...
device:
  # Runtime状态检测端口
  runtime:
    port: 1234              # 修改为实际Runtime监听端口

  # OPC UA服务端口
  opcua:
    serverPort: 4841        # 修改为实际OPC UA服务端口
```
无需重新编译程序，重启服务即可生效。

---

**总结**: 智能设备注册系统完美解决了多设备部署的核心痛点，实现了真正的即插即用和自动化管理。结合MQTT可靠性增强、Runtime状态监测和算法管理功能，构建了完整的企业级边缘设备管理解决方案，为大规模IoT部署提供了坚实的技术基础。
